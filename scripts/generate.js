...
async function generateThumb(srcPath, thumbRel) {
  ...
  if (!existsSync(thumbPath) || statSync(srcPath).mtimeMs > statSync(thumbPath).mtimeMs) {
    await sharp(srcPath).resize(THUMB_WIDTH).webp({ quality: 80 }).toFile(thumbPath)
  }
}
...
async function main() {
    ...
    if (topDir === 'Biome Images') {
      ...
      for (const sub of subs) {
        ...
        for (let i = 0; i < pngs.length; i += CONCURRENCY) {
          ...
          const processed = await Promise.all(batch.map(async (png) => {
  ...
  const totalImages = biomeGroups.reduce((s, g) => s + g.images.length, 0) +
  ...
  writeFileSync(DATA_FILE, JSON.stringify({ rawBase, biomes: biomeGroups, special: specialGroups }))
