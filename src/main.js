...
async function init() {
  ...
  const totalCount = biomes.reduce((s, b) => s + b.images.length, 0) + special.reduce((s, g) => s + g.images.length, 0)
...
function render() {
  ...
  document.querySelectorAll('.special-section .image-grid img').forEach(img => {
    img.addEventListener('click', () => openLightbox(img.dataset.original, img.src))
  })
...
function toggleBiome(name) {
  const targetEl = document.querySelector(`.biome-expanded[data-biome="${name}"]`);
  ...
  document.querySelectorAll('.biome-expanded.open').forEach(el => el.classList.remove('open'));
...
function openLightbox(original, thumbSrc) {
  const encoded = original.split('/').map(s => encodeURIComponent(s)).join('/')
  ...
  lightbox.innerHTML =
    ...
    (fullUrl
      ? `<a class="lb-link" href="${fullUrl}" target="_blank" rel="noopener">View Full Resolution</a>`
      : '') +
    '</div>'
...
function observeImages() {
  ...
  document.querySelectorAll('img:not(.loaded)').forEach(img => obs.observe(img))
}
